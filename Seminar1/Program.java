...
public class Program {
    ...
    static void task2() {
        while (true) { // Запускаем вечный цикл
            ...
            // Введенное число из строки превращаем число и помещаем в переменную
            int searchValue = Integer.parseInt(scanner.nextLine());
            // Генерируем случайный массив от 1 до 5
            int[] array = new int[random.nextInt(5) + 1];
            // Добавляем уловие в 33%, если размер ровняется 0, то стираем массив
            ...
            if (array != null) {
                for (int i = 0; i < array.length; i++) {
                    ...
                    System.out.printf("%d\t", array[i]); // Выводим результат через табуляцию
                }
                System.out.println();
            }
            // Теперь вызываем метод сортировки массива и поиска числа
    ...
    static int processArray(int[] array, int searchValue) {
        ...
        int searchResult = Arrays.binarySearch(array, searchValue); // Используем бинарный поиск
        if (searchResult < 0)
